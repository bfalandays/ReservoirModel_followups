...
ggsave('Figs/spikes_base.pdf', units="in",width=4,height=1.5)
...
ggsave('Figs/spikes_noise.pdf', units="in",width=4,height=1.5)
...
ggplot(meanspikes, aes(x = Time, y = pSpike)) + geom_line()+
  scale_y_continuous(name='Prop. Spiked')+
  scale_x_continuous(limits=c(0,2000))
ggsave('Figs/spikes_perturb.pdf', units="in",width=4,height=1.5)
#width = 800, height = 700
